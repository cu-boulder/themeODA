...
scale_fill_oda_gradient = function(low='black',mid=NA,high='gold',...){
  if (mid %in% names(oda_colors)){
    scale_fill_gradientn(colors=c(oda_colors[[low]],oda_colors[[mid]],oda_colors[[high]]),...)
  }

  else {
    scale_fill_gradient(low=oda_colors[[low]],high=oda_colors[[high]],...)
  }
}
